...
function DashboardContent() {
  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-16">
      ...
      <p className="text-[#6b6860] mb-10">Welcome back, {session.user.name ?? session.user.email}</p>

      {/* Purchase claim status */}
      {claimState === "claiming" && (
        <div className="bg-[#f0fdf4] border border-green-200 rounded-xl p-6 mb-6">
          <p className="text-[#16a34a] font-semibold">Processing your purchase…</p>
        </div>
      )}
      ...
      {(claimState === "success" || claimState === "already_claimed") && (
        <div className="bg-[#f0fdf4] border border-green-200 rounded-xl p-6 mb-6">
          <p className="text-[#16a34a] font-bold text-lg mb-1">
            {claimState === "success" ? "🎉 Purchase confirmed!" : "✅ Already activated"}
          </p>
          <p className="text-[#30302e] text-sm">
            Your <strong>{tier}</strong> access has been activated. Check your email — GitHub repository access will be granted automatically.
          </p>
        </div>
      )}
        ...
        <div className="p-6">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-[#30302e] font-medium">{session.user.name ?? "—"}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#6b6860]">Email</span>
              <span className="text-[#30302e] font-medium">{session.user.email}</span>
            </div>
          </div>
        </div>

        <div className="p-6">
          <h2 className="font-semibold text-[#30302e] mb-4">Repository Access</h2>
      ...
      {/* No purchase yet */}
      {claimState === "idle" && (
        <div className="mt-6 bg-white border border-[#e5e4df] rounded-xl p-6 text-center">
          <p className="text-[#6b6860] text-sm mb-4">You have not purchased access yet.</p>
          <a
            href="/pricing"
